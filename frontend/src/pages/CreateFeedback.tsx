...
/** Plain dropdown over the departments actually assigned to people in this
 * org — filters the person picker below it in "Review Participants" rather
 * than opening its own popup, since this list is usually short and a single
 * dropdown reads faster than a search panel for "pick one department".
 *
 * Deliberately NOT `/masters/departments`: that's the full org+global
 * master catalog, which can list departments nobody here has actually been
 * assigned yet (a global one added by a Super Admin, or one an org Admin
 * added preemptively). Picking one of those would filter the roster below
 * to zero people with no obvious reason why, so this calls the "in use"
 * endpoint instead, which only returns names that are on at least one
 * person's profile in this org. */
function DepartmentSelect({ value, onChange }: { value: string; onChange: (name: string) => void }) {
  ...
  useEffect(() => {
    api
      .get<{ items: DepartmentOption[] }>('/masters/departments/in-use')
